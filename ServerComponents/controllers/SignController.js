const { Account } = require('../models/index').default
...
class SignController {
    ...
    async find(req, res, next) {
        if (req.body.username != '') {
            ...
            res.json({ 'accessToken': accessToken, 'id': acc.id, 'refreshToken': refreshToken })
        }
    }
    ...
    async token(req, res, next) {
        ...
        const account = await Account.findOne({ where: { id: req.body.id } })
        ...
        try {
            ...
            res.json({'accessToken': newAccessToken, 'refreshToken': newRefreshToken, 'id': req.body.id})
        } catch (error) {
            console.log(error)
            res.sendStatus(403)
        }
    ...
    async register(req, res, next) {
        ...
        res.sendStatus(201)
        const { accessToken, refreshToken } = handlejwt(acc.dataValues)
        const find = await Account.findOne({ where: { username: req.body.username}})
        res.json({ 'accessToken': accessToken, 'id': find.id, 'refreshToken': refreshToken })
    }

    async logout(req, res, next) {
        let acc = await Account.findOne({ where: { id: req.id}})
